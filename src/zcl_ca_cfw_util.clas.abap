"! <p class="shorttext synchronized" lang="en">Common object: Helper methods for the control framework</p>
...
*   s t a t i c   m e t h o d s
    CLASS-METHODS:
      ...
      "! @parameter iv_soft_refresh | <p class="shorttext synchronized" lang="en">1 = Excluding sort, filter, etc.</p>
      alv_refresh_table_display
        IMPORTING
          io_alv_grid     TYPE REF TO cl_gui_alv_grid
          is_stable       TYPE lvc_s_stbl OPTIONAL
          iv_soft_refresh TYPE dml_boolean DEFAULT c_false,
      ...
      "! @parameter iv_set_mode_relative | <p class="shorttext synchronized" lang="en">1 = Set relative mode for row and cols (= Percent)</p>
      "! @parameter ro_splitter          | <p class="shorttext synchronized" lang="en">Created splitter container</p>
      create_splitter_ctrl
        IMPORTING
          io_parent            TYPE REF TO cl_gui_container
          iv_rows              TYPE i
          iv_columns           TYPE i
          iv_cnt_name          TYPE csequence OPTIONAL
          iv_set_mode_relative TYPE dml_boolean DEFAULT c_true
        RETURNING
      ...
      "!
      "! @parameter io_parent                     | <p class="shorttext synchronized" lang="en">CL_GUI_CONTAINER=&gt;SCREENx/DESKTOP / nothing for own dynpro</p>
      "! @parameter iv_display_only               | <p class="shorttext synchronized" lang="en">1 = Activate mode 'display-only'</p>
      "! @parameter iv_hide_statusbar             | <p class="shorttext synchronized" lang="en">1 = Hide status bar</p>
      "! @parameter iv_hide_toolbar               | <p class="shorttext synchronized" lang="en">1 = Hide tool bar</p>
      ...
      "! @parameter iv_wordwrap_to_linebreak_mode | <p class="shorttext synchronized" lang="en">1 = Word wrap at line number; 0 = Keep word wrap as is</p>
      ...
      create_text_edit_ctrl
        IMPORTING
          io_parent                     TYPE REF TO cl_gui_container
          iv_display_only               TYPE dml_boolean DEFAULT c_true
          iv_hide_statusbar             TYPE dml_boolean DEFAULT c_true
          iv_hide_toolbar               TYPE dml_boolean DEFAULT c_true
      ...
      "! @parameter rv_visible | <p class="shorttext synchronized" lang="en">1 = Control / container is visible</p>
      get_visible
        IMPORTING
          io_control        TYPE REF TO cl_gui_control
        RETURNING
          VALUE(rv_visible) TYPE dml_boolean,
      ...
      "! @parameter iv_cnt_name  | <p class="shorttext synchronized" lang="en">Name of container</p>
      link
        IMPORTING
          io_container    TYPE REF TO cl_gui_container
      ...
      "!
      "! @parameter io_control | <p class="shorttext synchronized" lang="en">Instance of control / container</p>
      "! @parameter iv_visible | <p class="shorttext synchronized" lang="en">1 = Display / 0 = Hide</p>
      set_visible
        IMPORTING
          io_control TYPE REF TO cl_gui_control
...
CLASS zcl_ca_cfw_util IMPLEMENTATION.
  ...
  METHOD alv_refresh_table_display.
    ...
    io_alv_grid->refresh_table_display(
                                  EXPORTING
                                    is_stable      = is_stable
                                    i_soft_refresh = xsdbool( iv_soft_refresh EQ c_true )
                                  EXCEPTIONS
                                    finished       = 1
                                    OTHERS         = 2 ).
    IF sy-subrc NE 0.
  ...
  METHOD create_cust_cont.
    ...
    IF sy-subrc NE 0.
      ...
      ENDIF.
    ENDIF.
  ENDMETHOD.                    "create_cust_cont


  METHOD create_diabox_cont.
    "-----------------------------------------------------------------*
    ...
    IF iv_cnt_name IS NOT INITIAL.
      set_name( io_control  = ro_diaboxcont
                iv_cnt_name = iv_cnt_name ).
    ENDIF.
  ENDMETHOD.                    "create_diabox_cont


  METHOD create_dock_cont.
    "-----------------------------------------------------------------*
    ...
    CREATE OBJECT ro_dockcont
      EXPORTING
        ...
        metric                      = cl_gui_docking_container=>metric_mm    "0    " Metric
        ratio                       = iv_ratio
        no_autodef_progid_dynnr     = xsdbool( iv_repid IS NOT INITIAL AND
                                               iv_dynnr IS NOT INITIAL )
        caption                     = iv_caption
    ...
    IF iv_cnt_name IS NOT INITIAL.
      set_name( io_control  = ro_dockcont
                iv_cnt_name = iv_cnt_name ).
    ENDIF.
  ENDMETHOD.                    "create_dock_cont


  METHOD create_splitter_ctrl.
    "-----------------------------------------------------------------*
    ...
    IF iv_set_mode_relative EQ c_true.
      ...
      set_column_mode( io_splitter = ro_splitter
                       iv_mode     = ro_splitter->mode_relative ).
    ENDIF.
  ENDMETHOD.                    "create_splitter_ctrl


  METHOD create_text_edit_ctrl.
    "-----------------------------------------------------------------*
    ...
    IF sy-subrc NE 0.
      ...
      ENDIF.
    ENDIF.
  ENDMETHOD.                    "create_text_edit_ctrl
  ...
  METHOD link.
    ...
    io_container->link(
                    EXPORTING
                      repid                       = iv_prog
                      dynnr                       = iv_dynnr
                      container                   = CONV char30( iv_cnt_name )
                    EXCEPTIONS
                      cntl_error                  = 1
                      cntl_system_error           = 2
  ...
  METHOD set_visible.
    ...
    CASE iv_visible.
      WHEN c_true.
        DATA(lv_visible) = cl_gui_control=>visible_true.
      WHEN OTHERS.
        lv_visible = cl_gui_control=>visible_false.
    ENDCASE.
